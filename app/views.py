    ...
    if request.method == "POST":
        ...
        if user:
            if user.is_authenticated:
                ...
                print(f"El usuario {correo} no está autenticado.")
                messages.error(request, "Hubo un problema con la autenticación.", extra_tags="error_inicio")
        else:
            print(f"Intento fallido de inicio de sesión para el correo: {correo}")
    ...
    try:
        if request.method == "POST":
            ...
            try:
                ...
                return render(request, "formularioRegistro.html", {"error": f"No se pudo enviar el mensaje de WhatsApp: {e}"})
